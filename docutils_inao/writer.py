...
class InaoWriter(writers.Writer):
    supported = ('text',)
    ...
    def translate(self):
        visitor = InaoTranslator(self.document)
...
class InaoTranslator(nodes.NodeVisitor):
    sectionchars = '*=-~"+`'
    ...
    def depart_list_item(self, node):
        ...
        if self.list_counter[-1] == -1:
            ...
            bullet = u'（%d）' % self.list_counter[-1]
            self.end_state(first=bullet, end=None)
